package evangel.hzcourse.chapter10.namecheck;
	...
	private class NameCheckScanner extends ElementScanner6<Void, Void> {
		...
		private void checkCamelCase(Element e, boolean initialCaps) {
			...
			if (Character.isUpperCase(firstCodePoint)) {
				previousUpper = true;
				if (!initialCaps) {
					messager.printMessage(WARNING, "名称“" + name + "”应当以小写字母开头",
							e);
					return;
				}
			} else if (Character.isLowerCase(firstCodePoint)) {
				if (initialCaps) {
					messager.printMessage(WARNING, "名称“" + name + "”应当以大写字母开头",
							e);
					return;
				}
			} else {
			...
			if (!conventional) {
				messager.printMessage(WARNING, "名称“" + name
