...
public class HttpsMockBase {
	...
	public static byte[] decrypt(byte data[], SecureRandom seed)
			throws Exception {
		// 对数据解密
		Cipher cipher = Cipher.getInstance(privateKey.getAlgorithm());
	...
	public static byte[] decryptByPublicKey(byte data[], SecureRandom seed)
			throws Exception {
		if (publicKey == null) {
	...
	public static byte[] decryptByDes(byte data[], SecureRandom seed)
			throws Exception {
		if (publicKey == null) {
			publicKey = CertifcateUtils.readPublicKeys();
		}
