...
public class ListenableFutureService {
	...
	public ListenableFutureService() {
		this.lExecService = MoreExecutors
	...
	public ListenableFuture<String> fetchConfig(String configKey) {
		return lExecService.submit(() -> {
			...
			return String.format("%s.%d", configKey,
	...
	public ListenableFutureTask<String> fetchConfigListenableTask(
			String configKey) {
	...
	public ListenableFuture<List<String>> getCartItems() {
		String[] items = new String[] { "Apple", "Orange", "Mango",
				"Pineapple" };
		return lExecService.submit(() -> {
			TimeUnit.MILLISECONDS.sleep(500);
			...
			if (noOfItems == 0)
				++noOfItems;
			return Arrays.stream(items, 0, noOfItems)
